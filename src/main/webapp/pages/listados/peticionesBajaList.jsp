...
<s:set var="titulo" value="cabecera.listado.peticiones_baja" />
<s:set var="contenido" />
...
<script type="text/javascript">
	...
	function filtra(pagina){
		...
		$.post('lista_peticiones_baja', datos, function(data){
			...
			$(data.listado).each(function(index){
				...
				+'<td style="text-align:center"><div class="ui-dt-c"><button id="j_idt46:tabla:0:j_idt62" name="j_idt46:tabla:0:j_idt62" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-left" onclick="darBaja('+
						this.id+');" type="button"><span class="ui-button-icon-left ui-icon ui-icon ui-icon-trash"></span><span class="ui-button-text">Dar de baja</span></button><button id="j_idt46:tabla:0:j_idt63" name="j_idt46:tabla:0:j_idt63" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-left" onclick="descartar('+
						this.id+');" type="button"><span class="ui-button-icon-left ui-icon ui-icon ui-icon-circle-close"></span><span class="ui-button-text">Descartar</span></button></div></td></tr>'
				</sec:authorize>).appendTo($('#j_idt52 tbody'));
	...
	function darBaja(id){
		if(confirm('Confirma para dar de baja')){
			var data = {
					id:id
			};
			$.post('peticiones_baja_baja', data, function(datos){
				$('#j_idt52 div.ui-messages').html('');
				if(!datos.error){
					$('<div class="ui-messages-info ui-corner-all"><span class="ui-messages-info-icon"></span><ul><li><span class="ui-messages-info-summary">Baja correcta</span><span class="ui-messages-info-detail">Los datos del socio han sido eliminados</span></li></ul></div>').appendTo($('#j_idt52 div.ui-messages'));	
				}else{
					$('<div class="ui-messages-info ui-corner-all"><span class="ui-messages-info-icon"></span><ul><li><span class="ui-messages-info-summary">Baja incorrecta</span><span class="ui-messages-info-detail">Los datos del socio han sido eliminados</span></li></ul></div>').appendTo($('#j_idt52 div.ui-messages'));
				}
				filtra();
			});
		}
	...
	function descartar(id){
		if(confirm('Confirma para dar descartar')){
			var data = {
					id:id
			};
			$.post('peticiones_baja_descartar', data, function(datos){
				$('#j_idt52 div.ui-messages').html('');
				if(!datos.error){
					$('<div class="ui-messages-info ui-corner-all"><span class="ui-messages-info-icon"></span><ul><li><span class="ui-messages-info-summary">Baja descartada</span><span class="ui-messages-info-detail">Petición de baja descartada</span></li></ul></div>').appendTo($('#j_idt52 div.ui-messages'));	
				}else{
					$('<div class="ui-messages-info ui-corner-all"><span class="ui-messages-info-icon"></span><ul><li><span class="ui-messages-info-summary">Baja incorrecta</span><span class="ui-messages-info-detail">Los datos del socio han sido eliminados</span></li></ul></div>').appendTo($('#j_idt52 div.ui-messages'));
				}
				filtra();
...
<div id="seccion" style="margin-top: 25px;">
	<s:text name="cabecera.listado.peticiones_baja"></s:text>
</div>
